...
"""Compare sequences of gene pairs, HUMAN adapted version based on paralog pairs analysis"""
...
def align_vsDB(paralog_pairs, fn_db, gene_uniq_uniprot, sequence_directory) -> None:
    """
    ...
    with open('vsAll_scores.csv', 'w') as csv_file:
        csv_file.write(f'sorted_gene_pair,A1,A2,uniprot_id_A1,uniprot_id_A2,A1_A2_rank,A2_A1_rank,A1_nb_selfSP,A2_nb_selfSP,A1_nb_taxid,A2_nb_taxid,A1_A2_bits,A2_A1_bits,A1_A2_id_percent,A2_A1_id_percent,A1_evalue,A2_evalue{r_line}')
        ...
        for index, row in df_paralogs_pairs.iterrows():
            ...
            for gene_id_i in [A1,A2]:
                ...
                # Unchanged - work with both uniprot mapping versions
                ...
                try:
                    vsDB_mmseqs(fn_seq_i, fn_db, row[paralog])
                except:
            ...
            if Path(f"mmseqs_{A2}.tsv").is_file() and len(uniprot_id_A1) > 0:
                ...
                print(f"mmseqs_{A2}.tsv", uniprot_id_A1)
                print(A1_row)
...
def main():
    ...
    # To compute alignments
        ...
        if args.db:
            align_vsDB(paralog_pairs, Path(args.db).resolve(), hgnc_infos, map_gene_uniprot, sequence_directory)
