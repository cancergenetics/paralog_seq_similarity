...
cols_to_use_uniprot = ['locus', 'uniprot']  # Specify the columns you need
...
#uniprot_to_symbol = df_uniprot_infos.set_index('uniprot')['locus'].to_dict()
uniprot_to_symbol = df_uniprot_infos.groupby('uniprot')['locus'].apply(list).to_dict()

# Load paralog gene pair to only write line for them
gene_pairs_full = pd.read_csv(PAIRS_FILE, usecols=['sorted_gene_pair'], sep=',')
...
for type_dist in models:
    ...
    # For multiple locus associated to a given uniprot
    for i in range(len(headers)):
        uniprot_i = headers[i].split('|')[1]
        locus_list_i = uniprot_to_symbol.get(uniprot_i, [])

        for j in range(len(headers)):
            uniprot_j = headers[j].split('|')[1]
            locus_list_j = uniprot_to_symbol.get(uniprot_j, [])
            
            # Generate all combinations of locus_i and locus_j
            for locus_i in locus_list_i:
                for locus_j in locus_list_j:
